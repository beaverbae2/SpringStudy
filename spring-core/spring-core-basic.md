...
- `@Configuration` : 설정(구성) 정보를 가지는 클래스
- `@Bean`
  - ``@Configuration` 이 선언된 클래스의 메소드에 선언, 해당 메소드에서 반환된 객체를 스프링 컨테이너에 스프링 빈으로 등록
  - 메소드명이 빈의 이름 -> `getBean()` 으로 호출 가능
...
- 스프링 컨테이너 생성 (구성 정보 활용)
  ...
  - value : 빈 객체
- 스프링 빈 등록
  - 구성 정보 내에 있는 메소드명을 key로, 메소드에서 리턴되는 객체를 value로 하여 빈 등록
-  스프링 빈 의존 관계 설정
   - 런타임 시 실제 생성된 빈들 간의 의존 관계가 동적으로 설정
